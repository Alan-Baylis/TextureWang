...
namespace NodeEditorFramework 
{
	...
	public static class NodeEditorSaveManager 
	{
		...
		public static NodeCanvas LoadNodeCanvas (string path, bool createWorkingCopy) 
		{
			...
			if (createWorkingCopy)
			{
			    while (nodeCanvas.nodes.Remove(null))
			    {
			        Debug.Log("removed a null node");
			    }
		...
		public static void CreateWorkingCopy (ref NodeCanvas nodeCanvas, NodeEditorState[] editorStates, bool compressed) 
		{
            ...
            Dictionary<string,NodeKnob> knobMap = new Dictionary<string, NodeKnob>();
			foreach (Node node in nodeCanvas.nodes)
			{
			    if (node == null)
			        continue;
...
//                    knobMap.Add(clonedKnob.m_LoadID,clonedKnob);
			...
			{ // Clone Nodes, structural content and additional scriptableObjects
				...
				{ // Clone generic NodeKnobs
					NodeKnob clonedKnob =  ReplaceSO (allSOs, clonedSOs, clonedNode.nodeKnobs[knobCnt]);
				    clonedNode.nodeKnobs[knobCnt] = clonedKnob;
...
//			    clonedNode.FixInternalKnobs(knobMap);
			}
		...
		private static T ReplaceSO<T> (List<ScriptableObject> scriptableObjects, List<ScriptableObject> clonedScriptableObjects, T initialSO) where T : ScriptableObject 
		{
		    ...
		    if (soInd == -1)
		    {
		        if (clonedScriptableObjects.IndexOf(initialSO)!=-1)
		        {
		            Debug.LogError("GetWorkingCopy: ScriptableObject " + initialSO.name +
		                           " is ALL ready a cloned object");
                    //already a cloned object
		            return initialSO;
		        }
		        else
		            Debug.LogError("GetWorkingCopy: ScriptableObject " + initialSO.name +
		                           " was not copied before! It will be null!");
		    }
