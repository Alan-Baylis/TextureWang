...
public class RuntimeNodeEditor : MonoBehaviour 
{
	...
	public void SideGUI () 
	{
		...
		{
			string path = UnityEditor.EditorUtility.SaveFilePanelInProject ("Save Node Canvas", m_LastLoadedName, "asset", "", NodeEditor.editorPath + "Resources/Saves/");
		...
		if (GUILayout.Button (new GUIContent ("Load Canvas", "Loads the Canvas from a Canvas Save File in the Assets Folder"))) 
		{
			...
			else
			{
			    ...
			    if (indexOf >= 0)
			            m_LastLoadedName = path.Substring(indexOf);
		...
		state.zoom = RTEditorGUI.Slider (new GUIContent ("Zoom", "Use the Mousewheel. Seriously."), state.zoom, 0.2f, 4);
